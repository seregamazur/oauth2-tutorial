...
const Checkout = () => {
  ...
  function toggleVisibility(methodId, methodType) {
    ...
    setSelectedDiv((prevSelectedDiv) => {
      ...
      document
        .getElementById(`payment-method-item-${methodType}`)
        .classList.remove('payment-method');
      document
        .getElementById(`payment-method-item-${methodType}`)
        .classList.add('payment-method--selected');
      return `payment-method-item-${methodType}`;
    });
    setRadioButtonState((prevState) => {
      document
        .getElementById(prevState)
        .classList.remove('payment-method__header__radio--selected');
      document
        .getElementById(`${methodType}-radio`)
        .classList.add('payment-method__header__radio--selected');
  ...
  async function blockNonSelectedMethods(methodType) {
    if (instantPaymentMethod !== methodType) {
      document
        .getElementById(`payment-method-${instantPaymentMethod}`)
        .classList.remove('instant-payment-method__details--opened');
      document
        .getElementById(`payment-method-${instantPaymentMethod}`)
        .classList.add('instant-payment-method__details--blocked');
    }
  ...
  function initializedPaymentMethod(method, index) {
    ...
    return (
      <li
        key={index}
        id={`payment-method-item-${method.type}`}
        onClick={() => {
      ...
      >
        <div className={'payment-method__header'}>
          <button className="payment-method__header__title" role={'radio'} type={'button'}>
            <span
              ...
              className={`${
                index === 0
                  ? 'payment-method__header__radio payment-method__header__radio--selected'
                  : 'payment-method__header__radio'
              }`}
            />
        ...
        <div
          id={divId}
          className={`${
            index === 0 ? 'payment-method__details--opened' : 'payment-method__details'
          }`}
        ></div>
      </li>
    );
  ...
  async function initPaymentMethods() {
    if (locationData) {
      ...
      if (paymentMethodsObject && Array.isArray(methods) && methods.length > 0) {
        ...
        const traditionalPaymentMethodDivs = methods
          .filter((m) => m.type !== instantPaymentMethod)
          .map((method, index) => {
        ...
        if (isInstantPaymentMethodAvailable(methods)) {
          setInstantPaymentMethodDiv(
            <div
              id="payment-method-googlepay"
              className={'instant-payment-method__details--opened'}
            />,
          );
        }
